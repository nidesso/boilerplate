    ...
    return (
        <Listbox as="div" className="w-full relative" value={props.value} onChange={props.valueChanged}>
                    <Listbox.Button className="bg-white border-solid border-gray-100 border px-4 py-2 rounded-lg relative w-full">
                        <span className="block truncate mr-4 text-gray-900 text-start">{(props.valueKey && props.value ? props.value[props.valueKey as keyof object] : props.value) as ReactNode}</span>
                    ...
                    >
                        <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                            {props.values.map((value) => (
                                <Listbox.Option
                                    key={(props.valueKey && props.value ? props.value[props.valueKey as keyof object] : props.value) as Key}
                                    value={value}
                                    ...
                                    {({ selected }) => (
                                        <Fragment>
                                            <span className={`block truncate ${selected ? 'font-medium' : 'font-normal'}`}>
                                                {(props.valueKey ? value[props.valueKey as keyof object] : value) as ReactNode}
                                            </span>
                                            {selected ? (
